/**
 * cashback-processor.js
 *
 * Claims cashback rewards for each account.
 *
 ...
 *      RECV: { mainID:100, subID:142, data:{ cashbackscore, blotterycashback, dynamicpass, score } }
 *            cashbackscore > 0 AND blotterycashback === 1 → cashback available
 *
 *   3. Claim cashback
 *      SEND: { userid, dynamicpass, mainID:100, subID:28 }
 *      RECV: { mainID:100, subID:144, data:{ result:0, lotteryscore, score } }
 *            result=0 → claimed, lotteryscore = amount received
 *
 * Error codes:
 *   result=-1 → IP banned — ban proxy, skip
...
class CashbackProcessor extends EventEmitter {
  constructor(db) {
    super();
    this.db              = db;
  ...
  async startProcessing(accountIds, repetitions = 1, useProxy = false, proxyList = []) {
    ...
    this.stats = {
      successCount: 0, failCount: 0, cashbackClaimed: 0,
      noCashback: 0, totalCashbackAmount: 0, activeWorkers: 0,
    ...
    this._emit('terminal', { type: 'info', message: '💰 CASHBACK CLAIM BOT STARTED' });
  ...
  async _runCycles() {
    while (this.isProcessing && this.currentCycle < this.totalCycles) {
      ...
      this._emit('cycleUpdate', {
        ...
        cashbackClaimed: this.stats.cashbackClaimed,
        totalCashbackAmount: this.stats.totalCashbackAmount,
      });
      this._emit('terminal', { type: 'success',
        message: `✅ Cycle ${this.currentCycle} done | Claimed: ${this.stats.cashbackClaimed} | Amount: ${this.stats.totalCashbackAmount}` });
  ...
  async _processWithRetry(account, globalIndex, attempt = 0) {
    ...
    try {
      ...
      await this.db.addProcessingLog(
        ...
        result.success
          ? `Cashback: ${result.cashbackClaimed ? `+${result.cashbackAmount || 0}` : 'not available'}`
          : result.error,
        result
      );
      ...
      if (result.success) {
        ...
        if (result.cashbackClaimed) {
          this.stats.cashbackClaimed++;
          this.stats.totalCashbackAmount += result.cashbackAmount || 0;
        } else {
          this.stats.noCashback++;
        }
      } else {
        this.stats.failCount++;
      }
  ...
  _accountFlow(account, index, attempt = 0) {
    return new Promise(async (resolve) => {
      ...
      // State machine: login → check → claim → done
      let phase         = 'login';
      let loginDone     = false;
      let cashbackAvail = false;
      let cashbackAmt   = 0;
      ...
      ws.on('message', (raw) => {
        ...
        if (msg.subID === 116 && !loginDone) {
          ...
          // Step 2 — check cashback availability
          phase = 'check';
          ws.send(JSON.stringify({
        ...
        if (msg.subID === 142 && phase === 'check') {
          ...
          cashbackAmt   = d.cashbackscore || 0;
          cashbackAvail = cashbackAmt > 0 && d.blotterycashback === 1;

          this._log(index, 'info',
            `💰 Cashback: ${cashbackAmt} | available: ${cashbackAvail} | blottery: ${d.blotterycashback}`);

          if (!cashbackAvail) {
            const reason = cashbackAmt === 0 ? 'no cashback earned' : 'cashback locked (blottery disabled)';
            this._log(index, 'info', `ℹ️ ${reason}`);
            return done({ success: true, cashbackClaimed: false, message: reason, newScore: lastScore });
          }

          // Step 3 — claim
          phase = 'claim';
          this._log(index, 'info', `💰 Claiming cashback: ${cashbackAmt}...`);
          ws.send(JSON.stringify({
            userid:      account.userid,
        ...
        // ── Cashback claim response subID:144 ───────────────────────────────
        if (msg.subID === 144 && phase === 'claim') {
          const d = msg.data || {};

          if (d.result === 0) {
            const amount = d.lotteryscore || cashbackAmt;
            const newBal = d.score !== undefined ? d.score : lastScore;
            this._log(index, 'success', `🎉 Cashback claimed: +${amount} | balance: ${newBal}`);
            return setTimeout(() => done({
              success: true, cashbackClaimed: true,
              cashbackAmount: amount, newScore: newBal,
            }), 300);
          } else {
            this._log(index, 'warning', `⚠️ Claim failed result=${d.result} (${d.msg || ''})`);
            return setTimeout(() => done({
              success: true, cashbackClaimed: false,
              message: `Claim failed: ${d.msg || 'unknown'}`,
              newScore: lastScore,
            }), 300);
          }
        }
  ...
  _complete() {
    ...
    this._emit('terminal', { type: 'success', message: '\n🎉 CASHBACK PROCESSING COMPLETED!' });
    this._emit('terminal', { type: 'info',    message: `📈 Success: ${this.stats.successCount} | Failed: ${this.stats.failCount}` });
    this._emit('terminal', { type: 'info',    message: `💰 Cashback claimed: ${this.stats.cashbackClaimed} | No cashback: ${this.stats.noCashback}` });
    this._emit('terminal', { type: 'info',    message: `💵 Total amount: ${this.stats.totalCashbackAmount}` });
...
module.exports = CashbackProcessor;
